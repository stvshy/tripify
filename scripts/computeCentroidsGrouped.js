...
// Importuj funkcję z mappera – dostosuj ścieżkę, jeśli potrzeba
const getCountryCodeByName = require('../utils/countryCodeMapper');

// --- Funkcje pomocnicze do obliczania centroidu --- //

/**
 * Funkcja wyciągająca punkty z atrybutu "d" ścieżki SVG.
 * Zakłada, że używane są absolutne komendy M i L.
 */
function extractPoints(d) {
  const points = [];
  // Wyrażenie dopasowujące polecenia "M" lub "L" (ignoruje "Z")
  const re = /[ML]([^MLZ]+)/gi;
  let match;
...
// Ścieżka do pliku SVG (dostosuj nazwę i ścieżkę, jeśli potrzeba)
...
fs.readFile(svgFilePath, 'utf8', (err, data) => {
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    // Przekształć surowy identyfikator na ujednolicony kod kraju
