...
const CountryProfile = () => {
  ...
  const [monthlyTempsVisible, setMonthlyTempsVisible] = useState(false);
  ...
  useEffect(() => {
    ...
    (async () => {
      const urls = await Promise.all(country.images.map(getFirebaseUrlCached));
      if (!cancelled) {
        setSliderUrls(urls);
      ...
      InteractionManager.runAfterInteractions(async () => {
        const [outlets, transports, drivingSide] = await Promise.all([
          Promise.all(country.outlets.map(getFirebaseUrlCached)),
          Promise.all(
        ...
        if (!cancelled) {
          ...
          setTransportUrls(filterNonEmpty(transports));
          setDrivingSideUrl(drivingSide);
        }
      });
    })();
  ...
  const toggleCountryVisited = useCallback(() => {
    ...
    showToast(
      isVisited
        ? "Country removed from visited list"
        : "Country added to visited list"
    );

    // Background update to Firebase
    updateDoc(doc(db, "users", auth.currentUser.uid), {
      countriesVisited: isVisited
        ? arrayRemove(countryId)
        : arrayUnion(countryId),
    }).catch((error) => {
  ...
  const positionReachedExtraInfo = (scrollY: number, contentHeight: number) => {
    // Tutaj określ miejsce, od którego ma się ładować extra info (np. 50% scrolla)
    return scrollY >= contentHeight / 3; // np. załaduj po przewinięciu 1/3 ekranu
  };
  return (
    <View style={{ flex: 1 }}>
      <ScrollView
      ...
      >
        ...
        <View style={styles.sliderContainer}>
          ...
          >
            <View
              style={[styles.tapArea, { left: 0, width: screenWidth * 0.2 }]}
            />
          </TapGestureHandler>
          ...
          >
            <View
              style={[styles.tapArea, { right: 0, width: screenWidth * 0.2 }]}
            />
          </TapGestureHandler>
                  ...
                  return (
                    <View
                      ...
                      style={[
                        ...
                        {
                          transform: [
                            { scale: 1.2 - distanceFromCenter * 0.2 },
                          ],
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity
            style={[
              styles.addButton,
              { backgroundColor: isVisited ? "rgb(6, 157, 11)" : "#a678e0" },
            ]}
            onPress={toggleCountryVisited}
          >
            <MaterialCommunityIcons
              name={isVisited ? "check" : "map-marker-plus-outline"}
              size={21}
              color="#fff"
            />
          </TouchableOpacity>
        ...
        {/* Sekcja rezerwująca miejsce na CountryExtraInfo */}
        <View
          style={{ minHeight: 300 }}
          onLayout={(event) => {
            extraInfoPositionY.current = event.nativeEvent.layout.y;
          }}
        >
          {showExtraInfo ? (
            <Suspense
              fallback={
                <View style={{ pointerEvents: "none", paddingVertical: 20 }}>
            ...
            <View style={{ height: 400 }} /> // przybliżona wysokość sekcji
          )}
        </View>
      </ScrollView>
